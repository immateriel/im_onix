...
class TestImOnix < Minitest::Test
  ...
  context "short tags" do
    ...
    should "have corporate name" do
      assert_equal "Julie Otsuka", @product.contributors.last.corporate_name
    end
  end
  ...
  context 'sales restriction of "Certaines n’avaient jamais vu la mer"' do
    ...
    should "have a global sales restriction" do
      assert_equal "09", @product.sales_restriction.type.code
    end
  
    should "have a market sales restriction" do
      assert_equal "LibraryEdition", @product.product_supplies.first.markets.first.sales_restrictions.first.type.human
    end
  end
  ...
  context "ONIX 2.1" do
    ...
    should "return the file's header as raw xml" do
      assert @message.raw_header_xml.to_s
    end
  
    should "return the product's raw xml" do
      assert @product.raw_xml.to_s
    end
    ...
    should "have empty list when no narrator" do
      assert_equal ["Test1 Tester", "Test2 Tester"], @product.narrators
    end
    ...
    should "have a sales restriction" do
      assert_equal "RetailerExclusiveOwnBrand", @product.sales_restrictions.first.sales_restriction_type.human
    end
  
    should "have a publishing status" do
      assert_equal "Active", @product.publishing_status.human
    end
  
    should "have a market publishing status" do
      assert_equal "Active", @product.market_representations.first.market_publishing_status.human
    end
